...
. The name of the file must not begin with an underscore (e.g., `_`).
. The file must not be located in a folder whose name begins with an underscore (e.g., `_`), except for posts.
. The document must begin with a http://jekyllrb.com/docs/frontmatter/[front matter header] (and cannot have a Byte Order Mark, or BOM).
