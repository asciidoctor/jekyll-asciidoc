...
For example, if the layout variable has the value `page`, Jekyll would look for a layout template at the path [path]__layout/page.html_.

If the layout is empty, the default layout is used (`default` for pages, `post` for posts, and the collection label for a document in a collection).
