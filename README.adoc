...
. The name of the file must not begin with an underscore (e.g., `_`).footnoteref:[ignored_files,This behavior is provided by Jekyll.]
. The file must not be located in a folder whose name begins with an underscore (e.g., `_`), except for posts.footnoteref:[ignored_files]
